...
def run_offline_quick_checks(project_root):
    ...
    def context_assembly_without_io():
        ...
        deps = {
            ...
            "get_persona_memory": lambda _text,
            _n_results=1: "【角色底色与自我认知】\n- 表达自然、重视边界",
            "get_long_term_memory": lambda *_args,
            **_kwargs: "【检测到相关背景记忆】\n- 用户最近在忙项目",
            "retrieve_bionic_memories": lambda *_args: "【仿生记忆】\n- 助手记得用户有点累",
            "get_user_profile_memory": lambda *_args: "【关于用户的稳定印象】\n- 用户重视效率",
    ...
    def command_service_without_io():
        ...
        audit_memories = [
            {
                "id": 11,
                "content": "助手正在协调客户行程，并提到用户总是忘记回复消息。",
                "theme": "生活",
                "emotion": "撒娇",
                "importance": 0.5,
            },
        ...
        check(
            "记忆审核列出疑似污染候选", "客户行程" in audit_payload and "标记遗忘" in audit_payload
        )
    ...
    def observation_runtime_state_priority():
        ...
        deps = {
            ...
            "get_presence_snapshot": lambda user_id: snapshot_store.get(user_id),
            "save_presence_snapshot": lambda user_id, snapshot: snapshot_store.update(
                {user_id: dict(snapshot)}
            )
            or True,
        ...
        replying_token = activate_presence_runtime_state(
            "_verify_runtime_user", "replying", deps=deps
        )
        ignored_token = activate_presence_runtime_state(
            "_verify_runtime_user", "reminder", deps=deps
        )
        ...
        check(
            "runtime_state observation 优先命中 replying", text.startswith("replying|正在回你消息|")
        )
        check(
    ...
    def observation_media_placeholder():
        ...
        base_snapshot = {
            ...
            "recent_chat_hint": "",
            "memory_hint": "",
            "observation_text": "助手像是被镜头定格住，周围光影正在一点点聚焦。",
            "media_type": "text",
            "media_status": "none",
            "media_prompt": "时间：下午；场景：镜头正在成像；动作：助手被光影定格",
            "media_key": "",
        ...
        deps = {
            "get_or_create_observation_snapshot": lambda user_id,
            force_refresh=False,
            deps=None: dict(base_snapshot),
            "save_presence_snapshot": save_snapshot,
            "get_presence_snapshot": lambda user_id: snapshot_store.get(user_id),
        ...
        check(
            "media_rendering runtime_state 已清理", "_verify_media_user" not in runtime_state_store
        )
            ...
            with patch(
                ...
                return_value={
                    ...
                    "snapshot": {
                        ...
                        "observation_text": "助手刚把手机扣在腿上，视线还在屏幕上停了停。",
    ...
    def health_payload_shape():
        ...
        check(
            "公开 health 不暴露 prompt 路径", "prompt_path" not in public_health.get("assets", {})
        )
    ...
    def provider_circuit_breaker():
        ...
        try:
            ...
            check(
                "半开探测成功后恢复 closed", recovered["providers"]["cerebras"]["state"] == "closed"
            )
        finally:
            reset_provider_circuits()
    ...
    results.append(
        run("offline.9 fallback 跳过已失败主模型", fallback_skips_already_failed_primary)
    )
    ...
    results.append(
        run("offline.9b 低成本 AI 释放 half-open probe", cheap_ai_releases_half_open_probe)
    )
    ...
    def persona_consistency_boundaries():
        ...
        bundled_private_scope = classify_reflection_scope(
            "哎呀，上次我们都去吃过了",
            "我还要处理客户行程，你还总记不住要及时吃饭。",
            "助手提到自己的客户行程，并提到用户总是忘记及时吃饭。",
        )
        assistant_schedule_scope = classify_reflection_scope(
            "你现在忙吗",
            "我还有客户行程要处理。",
            "助手提到自己的客户行程。",
        )
        user_travel_scope = classify_reflection_scope(
            "我的航班已经到了",
            "知道了。",
            "用户的航班已经到达。",
        )
        ...
        check(
            "助手临场行程不入仿生记忆",
            assistant_schedule_scope == "assistant_private_claim"
            and not should_save_reflection_scope(assistant_schedule_scope),
        )
        check(
            "用户明确出行信息仍可作为用户事实",
            user_travel_scope == "user_fact" and should_save_reflection_scope(user_travel_scope),
        )
        longing_prompt = build_prompt("我想死你啦")
        check(
            "强思念口语纠偏",
            "通常不是字面上的求死" in longing_prompt
            and "不要自行追加未配置的关系" in longing_prompt,
        )
        check("技术问答允许自然讨论", "可以用自然口吻帮用户分析" in build_prompt("帮我看代码报错"))
    ...
    def direct_current_time_reply_without_ai():
        ...
        check("时间回复只包含中性事实", "哼" not in reply and "还非要我报" not in reply)
        check(
            "时间回复只走普通文本发送",
        ...
        check(
            "时间回复使用中性本地摘要",
            reply_result["summary"] == {"intent": "报当前时间", "emotion": "平静", "theme": "日常"},
        )
    ...
    def multi_conversation_context_isolation():
        ...
        check(
            "清空默认上下文不影响新对话",
            get_recent_history(user_id)[-1]["content"] == "新对话里的第一句",
        )
    ...
    def story_mode_is_conversation_scoped():
        ...
        check("剧情设定进入提示", "雨夜，角色在约定地点与你碰面" in system_prompt)
...
def main():
    ...
    def bionic_state_crud():
        ...
        print(
            f"  mood={state['current_mood']}, stage={stage}, reflections={state2['total_reflections']}"
        )
    ...
    def ai_context_injection():
        ...
        for line in sys_msg.split("\n"):
            if "你当下的状态" in line or "你对用户的感觉" in line:
                print(f"  {line}")
    ...
    def context_layer_split():
        ...
        with (
            patch(
                "app.application.context_assembler.get_persona_memory",
                return_value="【角色】\n- 你嘴硬心软",
            ),
            patch(
                "app.application.context_assembler.get_long_term_memory",
                return_value="【长期记忆】\n- 用户最近总熬夜",
            ),
            patch(
            ...
            patch(
                ...
                return_value="【天气】: 上海今天小雨，气温 18 度",
            ),
    ...
    def search_explicit_failure():
        ...
        with patch.object(Config, "BOCHA_API_KEY", None):
            try:
                ...
                check(
                    "缺少 BOCHA_API_KEY 时显式抛错",
                    exc.degradation_reason == "bocha_api_key_missing",
                )
