...
_RUNTIME_STATE_PRESETS = {
    "replying": {
        "label": "正在回你消息",
        "scene_hint": "助手低头捧着手机敲字，指尖停停续续，像是在斟酌下一句该怎么回你",
        "ttl_seconds": 180,
        "priority": 40,
    },
    "media_rendering": {
        "label": "镜头正在成像",
        "scene_hint": "助手像是被一台看不见的镜头轻轻定格住，周围的光影正一点点聚焦成画面",
        "ttl_seconds": 600,
        "priority": 35,
    },
    "reflecting": {
        "label": "刚聊完还在回味",
        "scene_hint": "助手把手机搁在手边，视线却还没从刚才的对话里抽开，像是在慢慢回味你们刚说过的话",
        "ttl_seconds": 180,
        "priority": 30,
    },
    "proactive": {
        "label": "想主动戳你一下",
        "scene_hint": "助手拿着手机停顿了一会儿，删改几次后，像是决定主动给你发点什么",
        "ttl_seconds": 240,
        "priority": 20,
    },
    "reminder": {
        "label": "正惦记着提醒你",
        "scene_hint": "助手像是忽然想起什么似的拿起手机，敲下一句提醒",
        "ttl_seconds": 240,
        "priority": 10,
    },
}
...
def _build_recent_chat_hint(
    ...
    *,
    ...
    if delta_seconds <= _RECENT_CHAT_WINDOW_SECONDS:
        if latest_user_text:
            return f"助手刚看完你发来的“{latest_user_text}”，手机还扣在手边。", "recent_chat"
        return "助手刚把手机扣在手边，屏幕像是还停在和你的对话框上。", "recent_chat"

    if latest_user_text:
        return (
            f"助手隔一会儿会看一眼手机，像是在等关于“{latest_user_text}”的新消息。",
            "recent_chat",
        )
    return "助手隔一会儿会看一眼手机，像是在等新的消息。", "recent_chat"
...
def _build_fallback_observation(snapshot: Mapping[str, Any]) -> str:
    scene_detail = snapshot.get("scene_detail") or "助手安静待在自己的角落里"
    ...
    if not text:
        text = "助手这会儿安静待着，像是刚从一天的忙乱里留出一点自己的时间。"
...
def render_observation_text(
    snapshot: Mapping[str, Any], *, deps: Mapping[str, Any] | None = None
) -> tuple[str, str]:
    ...
    system_prompt = (
        ...
        "请用动作、环境、手部细节、表情和手机反应来表现助手当下的状态。\n"
        "称呼主体时只使用“助手”，不要推断性别。\n"
...
def get_or_create_observation_snapshot(
    ...
    *,
    ...
    snapshot = {
        ...
        "media_status": "none",
        "media_key": "",
        "updated_reason": active_runtime_state.get("state_code", "")
        if active_runtime_state
        else ("force_refresh" if force_refresh else "cache_miss"),
    ...
    *,
    ...
    return {
        ...
        "snapshot": {
            ...
            "generated_at": _to_local_iso(
                snapshot.get("generated_at", _format_sqlite_timestamp(utc_now_naive()))
            ),
            "expires_at": _to_local_iso(
                snapshot.get("expires_at", _format_sqlite_timestamp(utc_now_naive()))
            ),
        },
    }
