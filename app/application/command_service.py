...
from ..domain.persona_policy import get_persona_boundary_policy
...
def _handle_chat_command(
    ...
    get_active_conversation_fn = _get_dep(deps, "get_active_conversation", get_active_conversation)
    list_conversations_fn = _get_dep(deps, "list_conversations", list_conversations)
    set_active_conversation_fn = _get_dep(deps, "set_active_conversation", set_active_conversation)
    ...
    build_chat_current_card_fn = _get_dep(deps, "build_chat_current_card", build_chat_current_card)
...
def _default_story_scene() -> str:
    return "雨夜，角色在约定地点与你碰面，伞沿微微向你这边倾斜。"
...
def _memory_audit_reason(content: str, *, policy=None) -> str:
    policy = policy or get_persona_boundary_policy()
    matched = [marker for marker in policy.memory_audit_markers if marker in (content or "")]
    if matched:
...
def _get_memory_audit_candidates(
    ...
    *,
    ...
    candidates = []
    policy = get_persona_boundary_policy()
    for memory in memories:
        content = str(memory.get("content") or "")
        if not content:
            continue
        if should_include_memory_in_context("", content, policy=policy):
            continue
        candidate = dict(memory)
        candidate["reason"] = _memory_audit_reason(content, policy=policy)
...
def _handle_memory_command(
    ...
    send_feishu_fn = _get_dep(deps, "send_feishu", send_feishu)
    send_interactive_card_fn = _get_dep(deps, "send_interactive_card", send_interactive_card)
    build_memory_audit_card_fn = _get_dep(deps, "build_memory_audit_card", build_memory_audit_card)
    ...
    try:
        ...
        if action == "audit":
            ...
            if not candidates:
                send_feishu_fn(
                    open_id, "text", {"text": "记忆审核完成：没有发现明显的剧情污染候选。"}
                )
                return
