...
_ALLOWED_THEMES = frozenset((*_THEME_KEYWORDS, "其他"))
_ALLOWED_EMOTIONS = frozenset((*_EMOTION_KEYWORDS, "平静"))
...
def _build_reflection_fallback(user_text: str, assistant_reply: str) -> tuple[str, str, str, float]:
    ...
    content = f"用户提到“{preview}”，助手回应了这段表达并继续陪伴对话。"
...
def _do_reflect(user_id: str, user_text: str, assistant_reply: str) -> Optional[int]:
    ...
    system_prompt = (
        ...
        '  "scope": "从 shared_experience/user_fact/relationship_moment/assistant_private_claim/ephemeral 中选择。一起吃饭、一起玩、约会、见面、共同纪念优先 shared_experience；只来自助手回复的私人工作或行程优先 assistant_private_claim。"\n'
        "}"
    )
    user_prompt = f"【用户说】\n{user_text[:500]}\n\n【助手回复】\n{assistant_reply[:500]}"
    ...
    if not raw:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise TypeError("reflection payload must be an object")
            content_candidate = data.get("content")
            content = (
                content_candidate.strip()
                if isinstance(content_candidate, str) and content_candidate.strip()
                else fallback_content
            )
            theme_candidate = data.get("theme")
            theme = (
                theme_candidate
                if isinstance(theme_candidate, str) and theme_candidate in _ALLOWED_THEMES
                else fallback_theme
            )
            emotion_candidate = data.get("emotion")
            emotion = (
                emotion_candidate
                if isinstance(emotion_candidate, str) and emotion_candidate in _ALLOWED_EMOTIONS
                else fallback_emotion
            )
            importance = clamp_importance(float(data.get("importance", fallback_importance)))
        except (json.JSONDecodeError, ValueError, TypeError) as exc:
            logger.warning(
                "⚠️ 反思引擎 JSON 解析失败，改用本地兜底编码",
...
def get_runtime_state_context(user_id: str) -> str:
    ...
    parts = [
        ...
        f"你对用户的感觉：{describe_relationship_stage(stage)}",
    ]
    return "\n【内心的波澜与直觉】\n" + "\n".join(parts) + "\n"
