...
def _log_context_observability(
    *,
    ...
    if getattr(Config, "LOG_CONTENT_ENABLED", False):
        extra["context_previews"] = {
            name: _compact_preview(text) for name, text in context_blocks.items() if text
        }
    else:
        extra["context_chars"] = {name: len(text) for name, text in context_blocks.items() if text}
    logger.info("🔎 上下文命中概览", extra=extra)
...
def _format_story_block(story_scene: str) -> str:
    ...
    return (
        ...
        "可以自然描写场景、动作、环境和情绪推进，但要遵循系统提示词中的角色设定，表达自然且有分寸。"
...
def build_messages(
    ...
    *,
    ...
    story_scene = (
        active_conversation.get("summary") or active_conversation.get("title") or ""
    ).strip()
