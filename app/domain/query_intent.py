...
def _recent_assistant_has_private_life_claim(
    ...
    for item in reversed(history[-6:]):
        ...
        if any(marker in content for marker in policy.persona_private_life_markers):
            return True
    return False
...
def is_persona_private_life_query(
    ...
    *,
    ...
    if any(marker in text for marker in policy.user_self_life_markers):
        return False

    has_subject = any(marker in text for marker in policy.persona_private_subject_markers)
    has_private_marker = any(marker in text for marker in policy.persona_private_life_markers)
    if has_private_marker and _recent_assistant_has_private_life_claim(history, policy):
        return True
