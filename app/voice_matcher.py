...
def _sensitive_voice_fields(field_name: str, value) -> dict:
    """Build privacy-safe metadata for voice matching diagnostics."""
    return sensitive_text_fields(
        field_name,
        value,
        include_content=bool(Config.LOG_CONTENT_ENABLED),
    )
...
def _pick_existing_audio_path(
    ...
    for idx, matched_filename in enumerate(candidates):
        ...
        if not os.path.exists(audio_path):
            logger.warning(
                "⚠️ 命中语音文件不存在，已跳过",
                extra=_sensitive_voice_fields("voice_file", os.path.basename(matched_filename)),
            )
            continue
    ...
    if ranked_candidates:
        ...
        query_tags_text = ",".join(sorted(normalized_query_tags))
        logger.info(
            "🏷️ 语音候选重排完成",
            extra={
                "query_tag_count": len(normalized_query_tags),
                "best_tag_overlap": best_tag_overlap,
                "best_score": round(best_score, 3),
                **_sensitive_voice_fields("query_tags", query_tags_text),
                **_sensitive_voice_fields("voice_file", os.path.basename(matched_filename)),
            },
        )
...
def match_voice_file_with_diagnostics(
    ...
    if audio_collection is None:
        if fallback_path:
            fallback_fields = _sensitive_voice_fields(
            ...
            logger.warning(
                "⚠️ audio_collection 未初始化，使用保底语音",
                extra=fallback_fields,
            )
    ...
    try:
        ...
        for attempt_name, where_filter in _build_query_attempts(emotion, theme):
            ...
            if audio_path:
                query_tags_text = ",".join(sorted(query_tags))
                logger.info(
                    "✨ 语音匹配命中",
                    extra={
                        "attempt": attempt_name,
                        "distance": round(distance, 4) if distance is not None else None,
                        "query_tag_count": len(query_tags),
                        **_sensitive_voice_fields("core_intent", core_intent),
                        **_sensitive_voice_fields("emotion", emotion),
                        **_sensitive_voice_fields("theme", theme),
                        **_sensitive_voice_fields("query_tags", query_tags_text),
                        **_sensitive_voice_fields(
                            "voice_file", os.path.basename(matched_filename or "")
                        ),
                    },
                )
                return _build_voice_result(
        ...
        logger.error("语音匹配过程发生异常", extra=error_fields)
        if fallback_path:
            fallback_fields = _sensitive_voice_fields(
                "fallback_audio", os.path.basename(fallback_path)
            )
            ...
            logger.warning(
                "⚠️ 语音匹配异常，回退保底语音",
                extra={**fallback_fields, **error_fields},
            )
            return _build_voice_result(
                fallback_path,
                degraded=True,
