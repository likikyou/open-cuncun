...
from .domain.persona_policy import configure_persona_boundary_policy
from .entrypoints.scheduler_runner import run_scheduler
from .infrastructure.persona_policy_loader import load_persona_boundary_policy
...
def log_runtime_asset_status(*, config=Config, logger_obj=logger) -> None:
    ...
    logger_obj.info(
        ...
        extra={
            ...
            "memory_path_ok": os.path.isdir(config.MEMORY_PATH),
    ...
    if not prompt_ok:
        logger_obj.warning("⚠️ 提示词模板不可读")
    if not voice_lib_ok:
        logger_obj.warning("⚠️ 语音目录不可用或为空")
    if not card_image_ok:
        logger_obj.warning("⚠️ 卡片主图目录不可用或为空")
    if not audio_collection_ok:
        logger_obj.warning("⚠️ companion_audio 集合不可用")
...
def initialize_runtime(
    *,
    ...
    logger_obj=logger,
    init_db_fn=init_db,
    load_persona_policy_fn=load_persona_boundary_policy,
    configure_persona_policy_fn=configure_persona_boundary_policy,
    ...
    with _runtime_lock:
        if not _runtime_initialized:
            config.validate()
            persona_policy = load_persona_policy_fn(
                getattr(config, "PERSONA_POLICY_PATH", None),
                logger_obj=logger_obj,
            )
            configure_persona_policy_fn(persona_policy)
            init_db_fn()
...
def start_scheduler_thread(*, run_scheduler_fn=run_scheduler, logger_obj=logger) -> None:
    ...
    with _runtime_lock:
        if _scheduler_thread_started:
            return
        threading.Thread(
            target=run_scheduler_fn, daemon=True, name="feishu-companion-scheduler"
        ).start()
...
def start_app(
    ...
    *,
    ...
    logger_obj.info(f"🚀 Cuncun Core V{version} 启动成功: {host}:{port} debug={debug_mode}")
