...
def _get_optional_path_env(env_name: str) -> str | None:
    raw_value = os.getenv(env_name)
    if raw_value is None or not raw_value.strip():
        return None
    return _resolve_path(raw_value.strip())
...
class Config:
    ...
    # External vision is opt-in; Qwen is kept only as an explicit legacy provider.
    VISION_PROVIDER = os.getenv("VISION_PROVIDER", "safe_text").strip().lower()
    VISION_PROVIDER_FALLBACK = os.getenv("VISION_PROVIDER_FALLBACK", "safe_text").strip().lower()

    # Gemma vision provider and optional legacy shadow path.
    GEMMA_VISION_ENABLED = _get_bool_env("GEMMA_VISION_ENABLED", False)
    GEMMA_VISION_SHADOW_MODE = _get_bool_env("GEMMA_VISION_SHADOW_MODE", False)
    GEMMA_VISION_SHADOW_STORE_PREVIEWS = _get_bool_env("GEMMA_VISION_SHADOW_STORE_PREVIEWS", False)
    GEMMA_VISION_MODEL = os.getenv("GEMMA_VISION_MODEL", "gemma-4-31b")
    GEMMA_VISION_TIMEOUT_SECONDS = _get_positive_float_env("GEMMA_VISION_TIMEOUT_SECONDS", 15.0)
    GEMMA_VISION_MAX_IMAGE_MB = _get_positive_float_env("GEMMA_VISION_MAX_IMAGE_MB", 5.0)
    GEMMA_VISION_ALLOWED_MIME_TYPES = os.getenv(
        "GEMMA_VISION_ALLOWED_MIME_TYPES",
        "image/png,image/jpeg",
    )
    GEMMA_VISION_SHADOW_ARTIFACT_DIR = _get_path_env(
        "GEMMA_VISION_SHADOW_ARTIFACT_DIR",
        os.path.join("artifacts", "model_eval", "vision_shadow"),
    )
    ...
    PERSONA_POLICY_PATH = _get_optional_path_env("PERSONA_POLICY_PATH")
    ...
    # 默认日志只记录长度与摘要哈希；仅在受控排障时显式开启正文预览。
    LOG_CONTENT_ENABLED = _get_bool_env("LOG_CONTENT_ENABLED", False)
