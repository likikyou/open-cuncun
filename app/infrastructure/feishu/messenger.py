...
def send_feishu(receive_id: str, msg_type: str, content: dict) -> bool:
    ...
    if response and response.status_code == 200:
        ...
        logger.error(
            "❌ 发送消息业务失败",
            extra={"msg_type": msg_type, "error_code": result.get("code")},
        )
    return False
...
def send_random_emoticon(receive_id: str, emoticon_dir: str | None = None) -> bool:
    ...
    if not emoticon_files:
        logger.warning("⚠️ 表情包目录为空")
        return False

    chosen = random.choice(emoticon_files)
    logger.info("🎭 已随机选择表情包")
    ...
    if response and response.status_code == 200:
        ...
        if result.get("code") == 0:
            ...
            if message_id:
                ...
                return message_id
            logger.error("❌ 卡片消息发送成功但响应缺少 message_id")
        else:
            logger.error(
                "❌ 发送卡片消息业务失败",
                extra={"error_code": result.get("code")},
...
def stream_update_card_text(
    ...
    if response and response.status_code == 200:
        ...
        if result.get("code") != 0:
            logger.error(
                "❌ 流式更新业务失败",
                extra={"sequence": sequence, "error_code": result.get("code")},
            )
            return False
        return True
    logger.warning(
