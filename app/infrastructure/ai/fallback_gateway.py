...
class EmptyAIResponseError(RuntimeError):
    """Raised when a provider completes without usable text."""

    def __init__(self, provider_name: str) -> None:
        super().__init__(f"{provider_name} returned no text content")


def _response_text(response, provider_name: str) -> str:
    content = response.choices[0].message.content
    if not isinstance(content, str) or not content.strip():
        raise EmptyAIResponseError(provider_name)
    return content


def _iter_response_text(stream_obj, provider_name: str) -> Iterator[str]:
    pending_prefix = ""
    emitted_text = False
    ...
    for chunk in stream_obj:
        ...
        if not emitted_text:
            ...
            if not pending_prefix.strip():
                continue
            content = pending_prefix
            pending_prefix = ""
            emitted_text = True
        yield content
...
def build_kwargs(
    ...
    # GLM-family models may return null content when reasoning mode is enabled.
    # Keep public users on final-answer mode when they opt into such a model.
    if provider_name == "Cerebras" and "glm" in model.lower():
...
def call_with_fallback(
        ...
        def generate():
            ...
            if skip_primary:
                ...
                try:
                    ...
                    yield from _iter_response_text(stream_obj, primary_name)
            ...
            try:
                ...
                yield from _iter_response_text(stream_obj, fb_name)
    ...
    if not skip_primary:
        try:
            ...
            content = _response_text(response, primary_name)
    ...
    try:
        ...
        content = _response_text(response, fb_name)
...
class FallbackLLMGateway:
    """Adapter implementing the application LLM gateway port."""
