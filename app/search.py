    ...
    if not api_key:
        raise SearchUnavailableError(
            ...
            **sensitive_text_fields(
                "query", query, include_content=getattr(Config, "LOG_CONTENT_ENABLED", False)
            ),
        )
...
def should_search(user_text: str) -> bool:
    ...
    has_question = "?" in text or "？" in text or any(q in text for q in question_words)
