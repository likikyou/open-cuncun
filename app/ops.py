...
def _health_log_fields(health_data: Dict[str, Any]) -> Dict[str, Any]:
    """健康检查日志只保留聚合状态，不复制详情、路径或近期事件正文。"""
    observability = health_data.get("observability", {}) or {}
    degradations = observability.get("recent_degradations", {}) or {}
    ai_runs = observability.get("recent_ai_runs", {}) or {}
    return {
        "health_status": health_data.get("status", "unknown"),
        "components": dict(health_data.get("components", {}) or {}),
        "assets": _redact_assets(health_data.get("assets", {}) or {}),
        "recent_degradation_count": int(degradations.get("count", 0) or 0),
        "recent_ai_run_count": int(ai_runs.get("count", 0) or 0),
    }
...
def check_health(*, include_private: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        log_method(log_message, extra=_health_log_fields(health_data))
        return response_data
    except Exception as error:
        error_type = type(error).__name__
        logger.error("健康检查执行失败", extra={"error_type": error_type})
        response = {"status": "error", "message": "health check failed"}
        if include_private:
...
def backup_database_task() -> None:
    ...
    try:
        ...
        error_type = type(error).__name__
        logger.error("❌ 数据库备份失败", extra={"error_type": error_type})
        try:
            ...
            logger.error(
                "❌ 数据库备份告警发送失败",
                extra={"error_type": type(alert_error).__name__},
            )
