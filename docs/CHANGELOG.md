...
> 当前仓库状态：V5.8.0-rc.1 (2026-08-13)。

## Unreleased

### 开源上手与社区入口

- README 首屏补充项目定位、差异点、候选发布状态、当前限制与 CI/版本徽章，
  并统一 `uv run` 命令，避免使用系统 Python 时遗漏项目依赖。
- `.env.example` 的凭证与 API Key 改为空值，避免复制后把占位字符串误判为
  已配置密钥。
...
- 新增 `docs/OPEN_SOURCE_ROADMAP.md`，明确 Cuncun Core 的公开主线定位、
  仓库拆分边界、私有部署关系与公开发布规则。
- 新增公开发布安全审计脚本和 CI 门禁，阻止凭证、运行数据、未批准二进制素材与超大文件进入公开提交；私有部署标识
  通过仓外 blocklist 注入检测，仓库不保存这些敏感标识。
- 新增 `docs/CORE_API_V1.md`，固定未来 Console 与语音网关依赖的安全接口边界。
- 回迁公开安全的 AI 基础设施改进：新增 `app/ports/llm_gateway.py`、
  `FallbackLLMGateway` 适配器、`app/application/ai_summary_extractor.py`
  和 `app/application/ai_chat_runner.py`；摘要提取、同步工具循环和流式工具循环
  均已接入主 AI 边界，流式路径保留 `<think>...</think>` 过滤。
- 回迁可选 Gemma 视觉主路径：公开默认 `safe_text` 不上传图片；显式配置
  `VISION_PROVIDER=gemma` 与 `GEMMA_VISION_ENABLED=true` 后才走 Cerebras
  `gemma-4-31b`。Qwen 仅作为显式 legacy provider 保留；新增隐私收口的
  shadow JSONL 对比器、视觉 smoke tests 和 AI tool-runner smoke tests。
- 合并前兼容与隐私复审：恢复语音库既有的情绪/主题标签契约，将 Gemma
  默认图片白名单收紧为官方支持的 PNG/JPEG，并移除观察、主动消息、剧情和
  表情包链路中硬编码的私有人设；公开默认不绑定固定姓名、性别、职业或经历，
  部署者可通过系统提示词配置自己的角色。
- 角色边界规则收口为公共默认策略，并支持通过仓外 `PERSONA_POLICY_PATH` JSON
  追加私有部署标记；无效或缺失文件会安全回退，加载日志不记录路径或标记正文。
- 对话、上下文、搜索、工具参数、图片描述和异常 Webhook 的日志默认只记录长度与
  摘要哈希；`LOG_CONTENT_ENABLED=true` 仅供受控排障时短暂开启有限预览。
...
- 中文口语“想死你了”语义纠偏：运行时提示词会把这类表达识别为通常非字面的夸张思念，并按部署者的人格配置与上下文回应，避免字面误判或擅自追加关系设定。
...
- 人设一致性边界补强：私人生活问句不再触发 knowledge/web，默认天气地点改为“默认城市”，反思记忆会保留用户明确参与的共同经历并跳过助手临场编出的私人工作或出行安排。
...
1. **“想死你了”类中文口语语义纠偏**（`data/prompts/example_prompt_template.txt` + `app/prompt_builder.py`）
   提示词常驻规则把“想死你了/想你想死了/想你想到疯了”识别为通常非字面的夸张思念；`prompt_builder.py` 也会在命中这类输入时动态追加当轮语义提醒，并要求遵循部署人格与当前关系上下文，不自行推断恋爱、撒娇或其他关系设定。真实自伤/轻生风险词仍不会走这条纠偏逻辑。
...
10. **当前审计修复与离线冒烟补齐**（`app/entrypoints/feishu_webhook.py` + `tests/*_smoke.py` + `scripts/verify.py`）
...
11. **人设一致性边界补强**（`query_intent.py` + `reply_mode.py` + `context_policy.py` + `context_assembler.py` + `memory_rules.py` + `memory_reflection_service.py` + `example_prompt_template.txt`）
   新增“助手私人生活/工作/行程”问句识别，避免关系内提问误触发联网搜索或项目知识库；天气问句无明确地点时默认查“默认城市”；现实参考层明确禁止把外部新闻变成助手自己的工作、出行安排或私人经历。反思记忆新增 scope 分类：用户明确说“我们一起吃饭/一起玩/约会/见面”等共同经历时提高重要度并保留，只来自助手回复的私人工作、行程等临场剧情会跳过可检索仿生记忆。`tests/persona_policy_smoke.py` 与 `tests/runtime_persona_neutrality_smoke.py` 锁住这些边界。
...
| 版本 | 日期 | 摘要 |
|:---|:---|:---|
| V5.8.0-rc.1 | 2026-08-13 | Core Refresh：LLM ports、AI tool runner、隐私优先 Gemma 视觉、开源发布审计、仓库路线与 API v1 契约 |
