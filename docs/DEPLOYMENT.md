...
This guide covers deploying Cuncun Core to production.
...
```ini
[Unit]
Description=Cuncun Core Web Server
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/path/to/open-cuncun
Environment="PATH=/path/to/open-cuncun/.venv/bin"
ExecStart=/path/to/open-cuncun/.venv/bin/gunicorn \
  -w 1 --threads 8 -b 0.0.0.0:8081 wsgi:app
Restart=always
...
```ini
[Unit]
Description=Cuncun Core Scheduler
After=network.target
...
WorkingDirectory=/path/to/open-cuncun
Environment="PATH=/path/to/open-cuncun/.venv/bin"
ExecStart=/path/to/open-cuncun/.venv/bin/python run_scheduler.py
Restart=always
...
This server uses SSH to authenticate with GitHub for repository updates. Configure
the repository remote and verify the connection before pulling or pushing:

```bash
git remote set-url origin git@github.com:likikyou/open-cuncun.git
ssh -T git@github.com
git pull
git push
```

The GitHub account associated with the server's SSH key must have write access to
the repository. Never commit the private SSH key or other secrets.
