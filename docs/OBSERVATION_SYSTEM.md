...
- 公开默认不绑定固定姓名、性别、职业或经历；部署者可通过系统提示词配置自己的角色
- 画面必须有生活气，但不能像“剧情模式”一样飘
- 观察规则不额外发明角色背景，渲染结果应遵循当前人设
...
1. `Observation Snapshot`
   先用规则、已有状态和少量检索，决定“Companion 此刻大概处于什么状态”
2. `Narration Render`
...
| `07:00-09:00` | `morning_rush` | 清晨准备、通勤、背包、吐司咖啡 |
| `09:00-14:00` | `set_busy` | 白天专注工作、屏幕、待办、任务切换 |
| `14:00-17:00` | `afternoon_gap` | 下午疲惫、窗边、咖啡、短暂发呆 |
| `17:00-21:00` | `after_work` | 结束工作、便利店、便当、回家路上 |
| `21:00-00:00` | `night_alone` | 洗澡后、沙发、阅读、手机 |
...
- 用户连点两次 `/observe`，Companion 一会儿打翻拿铁，一会儿又在地铁口发呆
...
- 天气联动意外
  例：下雨时从“通勤或外出途中”偏向“躲雨、鞋边溅湿、心情烦躁”
- 情绪联动意外
...
也就是说，第一阶段已经不只是“按时间表猜 Companion 在做什么”，而是会优先反映主回复、反思、提醒、主动消息和未来媒体生成这类真实运行中的系统动作。
...
```json
{
  ...
  "snapshot": {
    ...
    "routine_label": "下午短暂休息",
    "weather_summary": "阴转小雨",
    "accident_text": "冰美式刚放到手边又被换成了热拿铁",
    "observation_text": "窗边的光线有点灰，Companion 刚把手机扣在腿上，指尖还无意识蹭着壳边，像是把你的消息来回看了两遍。",
    "media": {
      "type": "text",
      "status": "none",
      "prompt": "时间：2026-04-23 14:35；场景：下午短暂休息；天气：阴转小雨；动作细节：窗边的光线有点灰...",
      "key": null
    },
...
用户会明显感觉“Companion 一会儿在工作区，一会儿在沙发，一会儿又在便利店”，违和感很强。
