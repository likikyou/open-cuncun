...
- `ports/` 已引入 `LLMGateway` 协议，AI 摘要提取通过端口依赖基础设施实现
- `application/ai_summary_extractor.py` 已承接回复摘要提炼；`application/ai_chat_runner.py` 已承接同步/流式工具聊天循环
- 视觉入口已从单一 DashScope Qwen 调用改为 provider router：默认使用 `safe_text`，不上传图片；Gemma vision 与 legacy Qwen 仅在显式配置后启用，失败时安全文本降级
...
| `app/ai_engine.py` | 真实实现 | AI 主链编排、工具调用、fallback 边界与运行事件记录；摘要提炼已下沉到 `application/ai_summary_extractor.py` |
| `app/application/ai_summary_extractor.py` | 真实实现 | AI 回复摘要提炼，通过 `ports/LLMGateway` 端口依赖 AI 基础设施 |
| `app/application/ai_chat_runner.py` | 真实实现 | 同步/流式工具调用循环模块；主聊天路径已接入，流式输出仍在 `ai_engine.py` 过滤 `<think>` 内容 |
| `app/ports/llm_gateway.py` | 端口协议 | application 层使用的 LLM gateway 协议，由 `FallbackLLMGateway` 适配器实现 |
| `app/vision_gemma.py` | 真实实现 | Cerebras Gemma 视觉 provider |
| `app/vision_shadow.py` | 可选实现 | legacy Qwen 路径下的 Gemma shadow 对比器，只写脱敏 JSONL |
...
- `domain/query_intent.py` 会把对助手私人生活、工作或行程的关系内问题从普通 QA 里剥离出来，避免 `knowledge/web` 把外部新闻污染成助手自己的经历；天气问句无明确地点时默认查询“默认城市”。部署侧可通过仓外 persona policy JSON 追加自己的边界标记。
- `memory_reflection_service.py` 不再把只来自助手回复的私人工作、行程等临场剧情写入可检索仿生记忆；用户明确参与的共同吃饭、一起玩、约会、见面等经历会作为 `shared_experience` 提高重要度并保留。
...
- `.prompts/` 下的是开发协作提示词；运行时默认读取 `data/prompts/example_prompt_template.txt`，本地自定义文件需通过 `PROMPT_PATH` 显式选择。
