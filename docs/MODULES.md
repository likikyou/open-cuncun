...
> 最后整理：2026-04-23
> 当前仓库状态：V5.8.0-rc.1 (2026-08-13)，见 [CHANGELOG.md](CHANGELOG.md)。
...
| `VISION_PROVIDER` | `safe_text` | 图片理解 provider：默认不上传图片；显式选择 `gemma` / `qwen` 后才会把原图发送给对应第三方服务 |
| `VISION_PROVIDER_FALLBACK` | `safe_text` | Gemma 不可用时的生产 fallback；当前只支持安全占位文本 |
| `GEMMA_VISION_ENABLED` | `false` | Gemma 视觉总闸门；启用意味着图片会发送给 Cerebras，需先确认其服务条款和数据政策 |
| `GEMMA_VISION_SHADOW_MODE` | `false` | legacy Qwen 路径下是否异步运行 Gemma shadow 对比；默认关闭 |
| `GEMMA_VISION_SHADOW_STORE_PREVIEWS` | `false` | 是否在本地 shadow JSONL 中保存两侧描述预览；可能含 OCR/个人信息，仅限受控评测环境显式开启 |
| `GEMMA_VISION_MODEL` | `gemma-4-31b` | Cerebras 视觉模型 |
| `GEMMA_VISION_TIMEOUT_SECONDS` | `15` | Gemma 视觉请求超时秒数 |
| `GEMMA_VISION_MAX_IMAGE_MB` | `5` | Gemma 视觉允许的最大图片大小 |
| `GEMMA_VISION_ALLOWED_MIME_TYPES` | `image/png,image/jpeg` | Gemma 视觉 MIME 白名单 |
| `GEMMA_VISION_SHADOW_ARTIFACT_DIR` | `artifacts/model_eval/vision_shadow` | legacy shadow 结果目录；默认只存哈希、长度、状态和延迟，不保存原图/base64/描述预览 |
...
| `ALI_API_KEY` | 无 | DashScope Key；当前仅供 `VISION_PROVIDER=qwen` 的 legacy 视觉分支使用 |
...
| `PROMPT_PATH` | `data/prompts/example_prompt_template.txt` | 运行时人格提示词模板路径 |
| `PERSONA_POLICY_PATH` | 无 | 可选的部署侧角色边界 JSON 扩展；文件应放在仓库外或保持未跟踪，缺失/无效时回退公共默认 |
...
| `LOG_CONTENT_ENABLED` | `false` | 默认只记录敏感文本的长度与摘要哈希；设为 `true` 后会记录最多 120 字预览，仅应在受控排障时短暂开启 |
| `BACKUP_DIR` | `backups` | 备份目录 |

说明：相对路径会按 `PROJECT_ROOT` 自动解析。`PERSONA_POLICY_PATH` 指向的 JSON 只追加部署侧标记，不覆盖公共默认；加载日志不会输出文件路径或标记正文。示例结构如下：
...
| 文件 | 角色 | 说明 |
|:---|:---|:---|
| `pyproject.toml` | 当前工程配置入口 | 收口项目依赖、Python 版本约束、`Ruff` 基础配置与 dev 测试依赖（`pytest`） |
...
| `app/application/ai_summary_extractor.py` | `summarize_reply()`、`AISummaryDependencies` | 对话意图与情绪提取：从 AI 回复中提炼 `intent/emotion/theme` 供语音匹配；通过 `LLMGateway` 端口依赖 AI 基础设施 |
| `app/application/ai_chat_runner.py` | `run_sync_tool_chat()`、`iter_stream_tool_chat()`、`ChatToolRunState` | 同步/流式工具调用聊天循环模块；主聊天路径已接入，流式输出通过 `ai_engine` 保留 `<think>` 过滤 |
...
| `app/domain/persona_policy.py` | `PersonaBoundaryPolicy`、`get_persona_boundary_policy()` | 公共通用角色边界默认值，以及供领域规则共享的进程级纯数据策略 |
...
| 模块 | 关键入口 | 说明 |
|:---|:---|:---|
| `app/ports/llm_gateway.py` | `LLMGateway` | application 层依赖的 LLM 端口协议；当前由 `infrastructure.ai.FallbackLLMGateway` 实现 |
...
| 模块 | 关键入口 | 说明 |
|:---|:---|:---|
| `app/infrastructure/ai/__init__.py` | `get_active_client()`、`call_with_fallback()`、`FallbackLLMGateway`、`AIFallbackExhaustedError` | AI 基础设施公共导出入口 |
...
| `app/infrastructure/ai/fallback_gateway.py` | `call_with_fallback()`、`FallbackLLMGateway` | 统一 fallback 调用；主链已失败时可跳过二次主模型调用，主链耗尽时抛 `AIFallbackExhaustedError`，并记录降级与熔断事件；GLM 系列模型会自动加 `reasoning_effort=none` |

#### 部署策略

| 模块 | 关键入口 | 说明 |
|:---|:---|:---|
| `app/infrastructure/persona_policy_loader.py` | `load_persona_boundary_policy()` | 校验并加载未跟踪 JSON 扩展；缺失、过大、格式或 schema 无效时安全回退公共默认，日志不记录路径和标记正文 |
...
| `app/ai_engine.py` | LLM 调用、工具循环、摘要提炼，并记录请求/首包/工具/fallback 运行事件；摘要提炼已下沉到 `app/application/ai_summary_extractor.py` |
...
| `app/vision.py` | 图片分析统一入口：隐私优先默认 `safe_text`；Gemma 与 legacy Qwen 均需显式启用 |
| `app/vision_gemma.py` | Gemma 视觉 provider：Cerebras `gemma-4-31b` 图片理解，base64 data URI 输入，best-effort 返回结构化结果 |
| `app/vision_shadow.py` | Gemma shadow 对比：关闭即不调用/不落盘；默认 JSONL 只存哈希与运行元数据，描述预览需显式 opt-in，始终不存原图/base64 |
| `app/weather.py` | 天气查询 |
...
- 询问 Companion 是否休息、是否忙于工作或接下来去哪这类私人生活问题不应触发 knowledge/web；它们应该走关系内回答，避免外部信息污染 Companion 自己的经历。
...
- `memory_reflection_service.py` 会把反思分为 `shared_experience / user_fact / relationship_moment / assistant_private_claim / ephemeral`：用户明确参与的共同经历会提高重要度并保留，只来自 Companion 回复的私人工作或行程等临场信息不会写入可检索仿生记忆。
...
- `.prompts/`
  开发协作用提示词，不参与运行时对话。
- `data/prompts/example_prompt_template.txt`
  默认的公开安全人格模板，`prompt_builder.py` 会热加载 `PROMPT_PATH` 指向的文件。
- `data/prompts/prompt_template.txt`
  推荐的本地自定义文件名；复制模板后还需在 `.env` 中显式设置 `PROMPT_PATH`。
