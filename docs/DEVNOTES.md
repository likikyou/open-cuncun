...
# 完整测试（覆盖当前 tests/ 下的 smoke tests）
python -m pytest tests
...
> 本节是历史记录。下文出现的 `scripts/test_*`、迁移脚本或其他旧路径只描述当时的仓库结构，不是当前可执行命令；当前验证入口以第二节的 `python scripts/verify.py --offline` 和 `python -m pytest tests` 为准。
...
- `scripts/verify.py --offline` 新增多重对话隔离、剧情模式隔离、卡片中文化和记忆审核覆盖；当时的 `scripts/test_command_flow_smoke.py` 同步覆盖 `/chat`、`/story`、`/memory audit`。

### 2026-04-19：人设一致性边界补强

- `query_intent.py` 新增Companion私人生活问句识别和默认天气地点归一；“你那边天气”默认查“默认城市”。
- `reply_mode.py` / `context_policy.py` 避免“今天工作吗、你的客户是谁、私人行程”等关系内问题误触发 knowledge/web；混合“现在几点 + 天气”仍允许联网查天气。
- `context_assembler.py` 的现实参考层新增硬约束：外部新闻不能变成Companion自己的客户、出行安排或私人经历。
- `memory_rules.py` / `memory_reflection_service.py` 新增反思 scope：共同吃饭、一起玩、约会、见面等用户明确参与的经历会保留并提高重要度；只来自助手回复的工作、客户或行程等临场剧情会跳过可检索仿生记忆。
- `prompt_template.txt` 放开技术讨论边界：可以自然聊代码/报错/部署，但仍不能把自己说成程序或工具。
- 当时新增 `scripts/test_persona_consistency_smoke.py`，覆盖强思念口语、默认天气地点、私人生活不联网、共同经历记忆保护和技术问答边界。

### 2026-04-18：审计修复、smoke 迁移与离线验证补齐

- `entrypoints/feishu_webhook.py` 已补上解密空结果守卫，坏密文会明确返回 `400`，不再继续进入事件解析。
- 当时的 `scripts/test_*_smoke.py` 已从旧 facade 迁移到当时的分层模块，覆盖命令、消息、流式、调度、基础设施、上下文和 webhook 入口。
- `scripts/verify.py` 新增 `--offline` 快速验证模式，适合不碰真实 AI / embedding / 飞书接口时做本地回归。
- 当时建议的本地检查顺序是运行离线验证、Ruff 和相关 smoke `pytest`；当前命令以本文件第二节为准。
...
- `entrypoints/feishu_webhook.py` 去掉 `get_json(silent=True)`；非法 JSON 现在直接返回 `400`，日志默认只记录请求体长度与摘要哈希，方便在不持久化正文的前提下排查错误回调。
